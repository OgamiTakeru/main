...
def make_same_price_list_from_target_price(target_price, target_dir, peaks_all, predict_flag):
    ...
    """
    # ■通貨等に依存する数字
    dependence_same_price_range = 0.027  # 初期値（途中で変更される可能性あり）
    ...
    for i, item in enumerate(peaks_all):
        # 判定を行う
        # print(" 判定", item['time'], target_price - dependence_same_price_range, "<", item['peak'], "<=", target_price + dependence_same_price_range)
        if target_price - dependence_same_price_range <= item['peak'] <= target_price + dependence_same_price_range:
            ...
            if counter == 0:
                ...
                # print("target 変更 ", target_price, " ⇒", item['peak'], dependence_same_price_range)
...
def analysis_flag(dic_args):
    ...
    for i, each_predict_line_info in enumerate(
            predict_line_info_list):  # each_predict_line_infoはpredict_line_info_list_base同等
        ...
        if each_strength_info_result['line_on_num'] >= 2 and each_strength_info_result[
            'line_strength'] >= 0.9:  # allRangeは不要か
            # flag = judge_flag_figure_wrap_up(peaks, peaks[0]['direction'], each_strength_info_result['line_strength'], target_df)
            flag_info = judge_flag_figure_wrap_up_hard_skip(peaks, peaks[0]['direction'],
                                                            each_strength_info_result['line_strength'],
                                                            target_df)  # ★スキップバージョン
            ...
            if flag_info['tilt_flag']:
                ...
                each_strength_info_result['line_is_close_for_flag'] \
                    = True if dependence_line_close_border > abs(
                    each_predict_line_info['line_base_info']["line_base_price"] - peaks[0][
                        'peak']) else False  # breakラインまで近いかどうか

        else:
            print(s6, "flagtestなし", each_strength_info_result['line_on_num'],
                  each_strength_info_result['line_strength'])
...
def main_flag(dic_args):
    ...
    if flag_info['strength_info']['is_first_for_flag']:
        ...
        # 初回ではない場合
        dependence_normal_margin = 0.01
        dependence_lc_range_border = 0.08
        dependence_lc_range_at_least = 0.05
        dependence_counter_margin = 0.045
        dependence_lc_range_border_c = 0.083  # 0.12が強い
        ...
        if abs(lc_range) >= dependence_lc_range_border:
            # LCが大きすぎると判断される場合(10pips以上離れている）
            if lc_range < 0:
                # LC_rangeがマイナス値　＝　Directionは１。その為、現在価格からマイナスするとLCPriceとなる
                lc_price = target_price - abs(dependence_lc_range_border)
                print(s6, "ggLC価格(Dir1)", target_price, "-", abs(dependence_lc_range_border))
            else:
                # LC_rangeがプラス値　＝　Directionは-1。その為、現在書くにプラスするとＬＣＰｒｉｃｅになる
                lc_price = target_price + abs(dependence_lc_range_border)
                print(s6, "gLC価格(Dir-1)", target_price, "+", abs(dependence_lc_range_border))
        elif abs(lc_range) <= dependence_lc_range_at_least:
            # LCが小さすぎる
            if lc_range < 0:
                # LC_rangeがマイナス値　＝　Directionは１。その為、現在価格からマイナスするとLCPriceとなる
                lc_price = target_price - abs(dependence_lc_range_border)
                print(s6, "gggLC価格(Dir1)", target_price, "-", abs(dependence_lc_range_border))
            else:
                # LC_rangeがプラス値　＝　Directionは-1。その為、現在書くにプラスするとＬＣＰｒｉｃｅになる
                lc_price = target_price + abs(dependence_lc_range_border)
                print(s6, "gggggLC価格(Dir-1)", target_price, "+", abs(dependence_lc_range_border))
        else:
            # LCRangeが許容範囲内の場合、そのまま利用
            lc_price = temp_lc_price
        ...
        # 最大でもLCRange換算で10pips以内したい
        target_price = peaks[0]['peak'] - (dependence_counter_margin * flag_info['line_base_info'][
            'line_base_direction'])  # Nowpriceというより、取得価格
        # target_price = lc_price - (0.035 * flag_info['line_base_info']['line_base_direction'])  # 突破のLCから検討
        temp_lc_price = flag_info['strength_info']['flag_info']['oldest_peak_info']['oldest_info'][
            'peak']  # lcPriceは収束の中間点
        lc_range = temp_lc_price - target_price  # これがマイナス値の場合Directionは１、プラス値となる場合Directionは-1
        print(s6, "LC検討", target_price, temp_lc_price, lc_range, peaks[0]['peak'])
        if abs(lc_range) >= dependence_lc_range_border_c:
            # LCが大きすぎると判断される場合(10pips以上離れている）
            if lc_range < 0:
                # LC_rangeがマイナス値　＝　Directionは１。その為、現在価格からマイナスするとLCPriceとなる
                lc_price_counter = target_price + abs(dependence_lc_range_border_c)  # 通常とは符号逆 もともとー
                print(s6, "LC価格(Dir1)", target_price, "-", abs(dependence_lc_range_border_c))
            else:
                # LC_rangeがプラス値　＝　Directionは-1。その為、現在書くにプラスするとＬＣＰｒｉｃｅになる
                lc_price_counter = target_price - abs(dependence_lc_range_border_c)  # 通常とは符号逆　もともと＋
                print(s6, "LC価格(Dir-1)", target_price, "+", abs(dependence_lc_range_border_c))
        elif abs(lc_range) <= dependence_lc_range_at_least_c:
            # LCが小さすぎる
            if lc_range < 0:
                # LC_rangeがマイナス値　＝　Directionは１。その為、現在価格からマイナスするとLCPriceとなる
                lc_price_counter = target_price + abs(dependence_lc_range_border_c)
                print(s6, "LC価格(Dir1)", target_price, "-", abs(dependence_lc_range_border_c))
            else:
                # LC_rangeがプラス値　＝　Directionは-1。その為、現在書くにプラスするとＬＣＰｒｉｃｅになる
                lc_price_counter = target_price - abs(dependence_lc_range_border_c)
...
def for_practice_main_line_strength_analysis_and_order(dic_args):
    ...
    if flag_info['strength_info']['is_first_for_flag']:
        ...
        # 初回ではない場合
        # ■■フラッグ用（突破方向）
        main_order_base = cf.order_base_inspection(df_r.iloc[0]['close'],
                                                   df_r.iloc[0]['time_jp'])  # tpはLCChange任せのため、Baseのまま
        main_order_base['target'] = flag_info['line_base_info']['line_base_price'] + (
                    0.035 * flag_info['line_base_info']['line_base_direction'])  # 0.05
        main_order_base['lc'] = flag_info['strength_info'][
            'lc_price']  # 0.06 ←0.06は結構本命  # 0.09  # LCは広め　　  # 入れる側の文字は　LCのみ
        main_order_base['type'] = position_type
        main_order_base['expected_direction'] = flag_info['strength_info']['expected_direction']
        main_order_base['priority'] = 5  # flag_info['strength_info']['priority']
        main_order_base['units'] = main_order_base['units'] * 1
        ...
        # ■■カウンタオーダーも入れる（二回目以降のみ）
        # 最大でもLCRange換算で10pips以内したい
        now_price = peaks[1]['peak'] - (
                    0.035 * flag_info['line_base_info']['line_base_direction'])  # Nowpriceというより、取得価格
        temp_lc_price = flag_info['strength_info']['flag_info']['oldest_peak_info']['oldest_info'][
            'peak']  # lcPriceは収束の中間点
        lc_range = temp_lc_price - now_price  # これがマイナス値の場合Directionは１、プラス値となる場合Directionは-1
        ...
        main_order_base['target'] = peaks[1]['peak'] - (
                    0.035 * flag_info['line_base_info']['line_base_direction'])  # river価格＋マージン0.027
        # main_order_base['lc'] = gene.cal_at_most(0.09, flag_info['line_base_info']['line_base_price'] - 0.02 * (flag_info['strength_info']['expected_direction'] * -1))  # -0.02を追加してみた
        # main_order_base['lc'] = gene.cal_at_most(0.08, target_strength_info['line_base_info']['line_base_price']) # ←ダメだった！！！！
        # main_order_base['lc'] = flag_info['line_base_info']['line_base_price'] - 0.05 * (flag_info['strength_info']['expected_direction'] * -1)  # ←よかったけど、さらに上を！
        # gene.print_json(flag_info['strength_info']['flag_info']['oldest_peak_info']['oldest_info']['peak'])
