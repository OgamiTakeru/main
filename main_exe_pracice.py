...
def mode1():
    ...
    inspection_result_dic = im.for_inspection_analysis_warp_up_and_make_order(gl_data5r_df)
...
def exe_manage():
    ...
    # ■実行を行う
        ...
        if time_min % 5 == 0 and 15 <= time_sec < 40 and past_time > 60:  # キャンドルの確認　秒指定だと飛ぶので、前回から●秒経過&秒数に余裕を追加
        ...
        if gl_first_exe == 0:
            ...
            if d5_df['error'] == -1:
                ...
                d5_df = d5_df['data']
            # ↓時間指定
            jp_time = datetime.datetime(2024, 12, 24, 8, 15, 0)
