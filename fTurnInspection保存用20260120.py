...
class MainAnalysis:
    ...
    def predict_registance_turn_analysis(self):
        ...
        test=self.support_line_detect(1)
        print("SamePriceList")
        ...
        # 深いバスタブ型を確認
        print(s, "バスタブ型確認 (２点、最寄りの１つが30分以内、もう一つが５時間以上前、かつ　全部で３個以内")
    ...
    def simple_turn_analysis(self):
        ...
        r1 = river_df.iloc[1]  #
        print(r0['time_jp'], r0['body_abs'], r0['moves'])
        ...
        max_gap = max(gaps)
        min_gap = min(gaps)
        print("peak平均", avg_gap, peaks[0]['gap'],peaks[1]['gap'], peaks[2]['gap'])
        ...
        if river_cross_count >= river_cross_border and river['count'] >= 3:
            ...
            if lm_gap_ratio <= 0.4 and om_gap_ratio <= 0.4:
                print("対象のmiddle(2)は微折り返し認定⇒今回で二回目のため、レンジ警戒", lm_gap_ratio, om_gap_ratio)
                cross_range = True
            else:
                print("対象のmiddle(2)は微折り返しではない", lm_gap_ratio, om_gap_ratio)
                pass
    ...
    def support_line_detect(self, target_no):
        ...
        same_price_list0_total = sum(d['item']["peak_strength"] for d in same_price_list)
