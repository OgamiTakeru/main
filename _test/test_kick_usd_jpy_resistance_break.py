...
# 現行STOP版: M30/H1線は各足のnative FC2を30分/毎時境界で判定。TP/LC基準はM5。
# 下記の過去成績は旧M5トリガーの結果。native_m30_h1_fc2_v2出力とは別条件。
"""USD/JPY のM5・M30・H1抵抗線を「抜ける側」へ逆指値で入り、OOS 1年で測る。
...
スレッドではなく必ずプロセスを分けること。
    ...
    python test_kick_usd_jpy_resistance_break.py
"""
import _bootstrap
