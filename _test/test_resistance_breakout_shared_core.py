...
class ResistanceBreakoutProductionEquivalenceTest(unittest.TestCase):
    ...
    def _analysis_objects(self):
        ...
        m30_peaks = SimpleNamespace(
            ...
            peaks_original=[{
                ...
                "oldest_time_jp": (
                    self.decision_time - pd.Timedelta(minutes=60)
                ).strftime("%Y/%m/%d %H:%M:%S"),
            }],
        )
        h1_peaks = SimpleNamespace(
            ...
            peaks_original=[{
                ...
                "oldest_time_jp": (
                    self.decision_time - pd.Timedelta(hours=2)
                ).strftime("%Y/%m/%d %H:%M:%S"),
            }],
        )
        ...
        h1_bundle = CandleTimeframeBundle(
            timeframe="H1",
            duration=pd.Timedelta(hours=1),
            original_df_r=context.h1_original_df_r,
            completed_df_r=h1_completed_df_r,
            peaks_class=h1_peaks,
            source_granularity="H1",
        )
        self.h1_bundle = h1_bundle
        ...
        class FakeCandleAnalysis:
            ...
            def get_timeframe_bundle(
                ...
                bundles = {"M5": m5_bundle, "M30": m30_bundle, "H1": h1_bundle}
            ...
            def __init__(
                ...
                **options,
                ...
                trigger_direction = int(
                    bundle.peaks_class.peaks_original[0]["direction"]
                    if normalized in ("M30", "H1")
                    else self.context.newest_m5_peak["direction"]
                )
    ...
    def _validate(self, require_candidates=True, *, policy=None):
        ...
        with (
            ...
            patches[6],
            patches[7],
            patch.object(sweep, "_native_h1_bundle", return_value=self.h1_bundle),
        ):
            return sweep.validate_production_context_equivalence(
                ...
                [self.decision_index],
                sample_count=1,
                policy=self.EQUIVALENCE_POLICY if policy is None else policy,
    ...
    def test_sell_stop_side_also_matches(self):
        ...
        self.m30_bundle.peaks_class.peaks_original[0]["direction"] = -1
    ...
    def test_m30_fc2_does_not_require_m5_fc2(self):
        self.context.newest_m5_peak["count"] = 3

        result = self._validate(require_candidates=False)

        self.assertEqual(
            result["checked_candidates_by_timeframe"], {"M5": 0, "M30": 1},
        )
        self.assertEqual(result["mismatches"], 0)

    def test_m5_fc2_does_not_substitute_for_m30_fc2(self):
        self.m30_bundle.peaks_class.peaks_original[0]["count"] = 3

        result = self._validate(require_candidates=False)

        self.assertEqual(
            result["checked_candidates_by_timeframe"], {"M5": 1, "M30": 0},
        )

    def test_opposite_native_directions_are_independent(self):
        self.m30_bundle.peaks_class.peaks_original[0]["direction"] = -1
    ...
    def test_m30_is_not_repeated_at_the_next_m5_decision(self):
        self.context.decision_time += pd.Timedelta(minutes=5)
        with patch("fLineAnalysis.LineStrengthCal", self._fake_line_class()):
            orders = live_breakout.build_orders_for_decision(
                self.fake_ca, "inspection", policy=self.EQUIVALENCE_POLICY,
            )
        self.assertEqual(
            [order.exe_order_plan["line_timeframe"] for order in orders], ["M5"],
        )

    def test_sweep_rebuild_rejects_off_boundary_native_m30(self):
        self.context.decision_time += pd.Timedelta(minutes=5)
        self.m5.loc[self.decision_index, "time_jp_dt"] = self.context.decision_time
        with patch.object(sweep, "_native_m30_bundle", return_value=self.m30_bundle):
            with self.assertRaisesRegex(ValueError, "M30_decision_not_due"):
                sweep.rebuild_candidates_at(
                    self.m5, self.decision_index, self.pair_name,
                    m30=self.m30, h1=self.h1, decision_context=self.context,
                    entry_mode="stop", line_timeframe="M30",
                )

    def test_stale_m30_fc2_cannot_be_reused_at_new_boundary(self):
        self.m30_bundle.completed_df_r.drop(index=0, inplace=True)
        with patch.object(sweep, "_native_m30_bundle", return_value=self.m30_bundle):
            with self.assertRaisesRegex(sweep.CandleHistoryNotReady, "M30"):
                sweep.rebuild_candidates_at(
                    self.m5, self.decision_index, self.pair_name,
                    m30=self.m30, h1=self.h1, decision_context=self.context,
                    entry_mode="stop", line_timeframe="M30",
        ...
        with self.assertRaisesRegex(sweep.CandleHistoryNotReady, "M30"):
            self._validate(require_candidates=False)

    def test_native_m30_prefilter_ignores_forming_and_future_prices(self):
        m30 = self._frame(pd.date_range(
            start=self.decision_time - pd.Timedelta(minutes=90),
            periods=6, freq="30min",
        ), 20.0)
        m30["middle_price"] = [150.0, 150.1, 150.0, 150.2, 150.0, 150.2]
        altered = m30.copy()
        altered.loc[altered["time_jp_dt"] >= self.decision_time, "middle_price"] = 999.0
        end = self.decision_time + pd.Timedelta(minutes=5)
        expected = sweep.native_m30_count2_indices(
            self.m5, m30, self.decision_time, end,
        )
        actual = sweep.native_m30_count2_indices(
            self.m5, altered, self.decision_time, end,
        )
        self.assertEqual(expected, [self.decision_index])
        self.assertEqual(actual, expected)

    def test_native_m30_prefilter_excludes_explicit_incomplete_candle(self):
        m30 = self._frame(pd.date_range(
            start=self.decision_time - pd.Timedelta(minutes=90),
            periods=3, freq="30min",
        ), 20.0)
        m30["middle_price"] = [150.0, 150.1, 150.0]
        m30["complete"] = [True, True, False]
        result = sweep.native_m30_count2_indices(
            self.m5, m30, self.decision_time,
            self.decision_time + pd.Timedelta(minutes=5),
        )
        self.assertEqual(result, [])

    def test_native_m30_prefilter_does_not_discard_weekend_holiday_fc2(self):
        times = pd.DatetimeIndex([
            "2023-12-23 06:00:00", "2023-12-23 06:30:00",
            "2023-12-26 07:00:00",
        ])
        m30 = self._frame(times, 20.0)
        m30["middle_price"] = [150.0, 150.1, 150.0]
        decision = times[-1] + pd.Timedelta(minutes=30)
        m5 = self._frame(pd.DatetimeIndex([decision]), 10.0)
        result = sweep.native_m30_count2_indices(
            m5, m30, decision, decision + pd.Timedelta(minutes=5),
        )
        self.assertEqual(result, [0])

    def test_m30_pending_expiry_uses_next_m30_fc2(self):
        m5 = self._frame(pd.date_range(
            start=self.decision_time, periods=13, freq="5min",
        ), 10.0)
        m30 = self._frame(pd.date_range(
            start=self.decision_time - pd.Timedelta(minutes=90),
            periods=5, freq="30min",
        ), 20.0)
        m30["middle_price"] = [150.0, 150.1, 150.0, 150.1, 150.0]
        with patch.object(sweep.win_point, "candidate_indices", return_value=pd.Series([1, 2])):
            events = sweep.build_sweep_event_schedule(
                m5, m30, self.decision_time,
                self.decision_time + pd.Timedelta(minutes=65), "stop",
            )
        native = [event for event in events if event["line_timeframe"] == "M30"]
        self.assertEqual([event["index"] for event in native], [0, 6, 12])
        self.assertEqual(native[0]["next_count2_time"], self.decision_time + pd.Timedelta(minutes=30))
        self.assertIsNone(native[-1]["next_count2_time"])
        self.assertTrue(all(event["trigger_timeframe"] == "M30" for event in native))

    def test_limit_mode_keeps_legacy_m5_trigger_for_m30(self):
        with patch.object(sweep.win_point, "candidate_indices", return_value=pd.Series([170, 175])):
            events = sweep.build_sweep_event_schedule(
                self.m5, self.m30,
                self.decision_time - pd.Timedelta(hours=2),
                self.decision_time + pd.Timedelta(minutes=5), "limit",
            )
        self.assertEqual(len(events), 6)
        self.assertTrue(all(event["trigger_timeframe"] == "M5" for event in events))
        native_lines = [event for event in events if event["line_timeframe"] == "M30"]
        self.assertEqual([event["index"] for event in native_lines], [170, 175])

    def test_h1_fc2_uses_own_direction_without_m5_or_m30_fc2(self):
        self.context.newest_m5_peak["count"] = 3
        self.m30_bundle.peaks_class.peaks_original[0]["count"] = 3
        self.h1_bundle.peaks_class.peaks_original[0].update({"count": 2, "direction": -1})
        result = self._validate(
            policy=replace(self.EQUIVALENCE_POLICY, timeframes=("H1",)),
        )
        self.assertEqual(result["checked_candidates_by_timeframe"], {"H1": 1})
        self.assertEqual(result["peak_history_bars_by_timeframe"], {"H1": 240})
        self.assertEqual(result["mismatches"], 0)

    def test_all_three_native_timeframes_match_production_at_hour(self):
        self.h1_bundle.peaks_class.peaks_original[0]["count"] = 2
        result = self._validate(
            policy=replace(self.EQUIVALENCE_POLICY, timeframes=("M5", "M30", "H1")),
        )
        self.assertEqual(
            result["checked_candidates_by_timeframe"], {"M5": 1, "M30": 1, "H1": 1},
        )
        self.assertEqual(result["mismatches"], 0)

    def test_m5_fc2_does_not_substitute_for_h1_fc2(self):
        self.h1_bundle.peaks_class.peaks_original[0]["count"] = 3
        result = self._validate(
            require_candidates=False,
            policy=replace(self.EQUIVALENCE_POLICY, timeframes=("H1",)),
        )
        self.assertEqual(result["checked_candidates_by_timeframe"], {"H1": 0})

    def test_h1_is_not_repeated_at_half_hour_boundary(self):
        self.context.decision_time += pd.Timedelta(minutes=30)
        self.h1_bundle.peaks_class.peaks_original[0]["count"] = 2
        with patch.object(self.fake_ca, "get_timeframe_bundle", wraps=self.fake_ca.get_timeframe_bundle) as get_bundle:
            orders = live_breakout.build_orders_for_decision(
                self.fake_ca, "inspection",
                policy=replace(self.EQUIVALENCE_POLICY, timeframes=("H1",)),
            )
        self.assertEqual(orders, [])
        get_bundle.assert_not_called()

    def test_stale_h1_fc2_is_not_reused_at_new_hour(self):
        self.h1_bundle.peaks_class.peaks_original[0]["count"] = 2
        self.h1_bundle.completed_df_r.drop(index=0, inplace=True)
        with patch.object(sweep, "_native_h1_bundle", return_value=self.h1_bundle):
            with self.assertRaisesRegex(sweep.CandleHistoryNotReady, "H1"):
                sweep.rebuild_candidates_at(
                    self.m5, self.decision_index, self.pair_name,
                    m30=self.m30, h1=self.h1, decision_context=self.context,
                    entry_mode="stop", line_timeframe="H1",
                )
        with self.assertRaisesRegex(sweep.CandleHistoryNotReady, "H1"):
            self._validate(
                require_candidates=False,
    ...
    def test_native_h1_prefilter_ignores_forming_future_and_incomplete_rows(self):
        h1 = self._frame(pd.date_range(
            start=self.decision_time - pd.Timedelta(hours=3), periods=5, freq="h",
        ), 30.0)
        h1["middle_price"] = [150.0, 150.1, 150.0, 150.2, 150.0]
        end = self.decision_time + pd.Timedelta(minutes=5)
        expected = sweep.native_h1_count2_indices(self.m5, h1, self.decision_time, end)
        altered = h1.copy()
        altered.loc[altered["time_jp_dt"] >= self.decision_time, "middle_price"] = 999.0
        self.assertEqual(expected, [self.decision_index])
        self.assertEqual(
            sweep.native_h1_count2_indices(self.m5, altered, self.decision_time, end), expected,
        )
        h1["complete"] = True
        h1.loc[2, "complete"] = False
        self.assertEqual(
            sweep.native_h1_count2_indices(self.m5, h1, self.decision_time, end), [],
        )

    def test_h1_pending_expiry_uses_next_h1_fc2(self):
        m5 = self._frame(pd.date_range(
            start=self.decision_time, periods=25, freq="5min",
        ), 10.0)
        h1 = self._frame(pd.date_range(
            start=self.decision_time - pd.Timedelta(hours=3), periods=5, freq="h",
        ), 30.0)
        h1["middle_price"] = [150.0, 150.1, 150.0, 150.1, 150.0]
        with patch.object(sweep.win_point, "candidate_indices", return_value=pd.Series([1, 2])):
            events = sweep.build_sweep_event_schedule(
                m5, self.m30, self.decision_time,
                self.decision_time + pd.Timedelta(minutes=125), "stop", h1=h1,
            )
        hourly = [event for event in events if event["line_timeframe"] == "H1"]
        self.assertEqual([event["index"] for event in hourly], [0, 12, 24])
        self.assertEqual(hourly[0]["next_count2_time"], self.decision_time + pd.Timedelta(hours=1))
        self.assertIsNone(hourly[-1]["next_count2_time"])
        self.assertTrue(all(event["trigger_timeframe"] == "H1" for event in hourly))
