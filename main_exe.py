...
class main():
    def __init__(self):
        ...
        self.need_df_num = 100  # 解析には100行必要
        # 回数カウント系
        self.trade_num = 1
...
#
#     # ■調査実行
#     inspection_result_dic = gl_target_function(gl_data5r_df)
#     # ■ オーだ発行
...
#     else:
#         # オーダーを登録＆発行する
#         exe_res = positions.order_add(inspection_result_dic['exe_orders'])
#         if exe_res == 0:
#             tk.line_send(" オーダー発行失敗　main 368")
#         else:
#             tk.line_send("★★★オーダー発行", gl_trade_num, "回目: ", " 　　　", exe_res,
#                          ", 現在価格:", str(gl_now_price_mid), "スプレッド", str(gl_now_spread),
#                          "直前の結果:", classPosition.order_information.before_latest_plu, ",開始時間",
#                          gl_start_time_str)
#
...
#
# oa = self.oa = classOanda.Oanda(tk.accountIDl, tk.access_tokenl, tk.environmentl)
# # ■処理の開始
# positions = classPositionControl.position_control(True)  # ポジションリストの用意
# positions.reset_all_position()  # 開始時は全てのオーダーを解消し、初期アップデートを行う
#
# gl_target_function = am.wrap_all_inspections  # 利用する調査関数
#
#
